...
def queued_orbit_search(init_graph, local_ops, save_edges, verbose):
    ...
    while queue:
        ...
        for rep_node, equiv_nodes in list(node_equivs.items()):
...
def get_min_edge_reps(class_graph):
    ...
    min_edge_reps = {key: graph for key, graph in list(class_graph.nodes.items())
...
def get_max_edge_reps(class_graph):
    ...
    max_edge_reps = {key: graph for key, graph in list(class_graph.nodes.items())
...
def export_class_graph(class_graph, filename, min_edge_reps=False):
    ...
    for node, attrs in list(class_graph.nodes.items()):
