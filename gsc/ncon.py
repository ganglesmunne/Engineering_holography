...
def QutipToNumpy(State):
	
	N=int(np.log2(State.shape[0]))
	dim=State.dims[0]
	T=np.zeros(dim,dtype=complex)
	for b in itertools.product([0, 1], repeat=N):
		bi=tuple(np.ones(N,dtype=int)-np.array(b))
		T[b]=(qt.basis(dim,list(bi)).dag()*State)[0][0][0]
	return T
...
def NumpyToQutip(T):
	...
	N=len(T.shape)
	dim=list(T.shape)
	State=0*qt.basis(dim,list(np.zeros(N,dtype=int)))
	
	for b in itertools.product([0, 1], repeat=N):
		bi=tuple(np.ones(N,dtype=int)-np.array(b))
		State = State + T[b]*qt.basis(dim,list(bi))
